    ...
    <div className="border rounded-xl p-4 shadow bg-white text-black dark:bg-gray-800 dark:text-white">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex gap-1">
          <button onClick={() => onMoveUp(char)}>🔼</button>
          <button onClick={() => onMoveDown(char)}>🔽</button>
          <button onClick={() => onRemove(char)} className="text-red-500">삭제</button>
        </div>
      </div>
          ...
          {taskSets[type]?.map((task) => (
            <div key={task} className="inline-flex items-center m-1">
              <button
                ...
                className={`px-2 py-1 rounded border ${data[type]?.[task] ? "bg-green-500 text-white" : "bg-white text-black"}`}
              >
                {task}
              </button>
              <button
